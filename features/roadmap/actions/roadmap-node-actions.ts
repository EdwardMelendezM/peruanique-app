...
export async function createRoadmapNode(
  ...
  if (!group) {
    return { success: false, error: "El grupo no existe", fieldErrors: { groupId: "Grupo inválido" } };
  }

  if (!lesson) {
    return { success: false, error: "La lección no existe", fieldErrors: { lessonId: "Lección inválida" } };
  }

  const uniqueOrderError = await validateUniqueOrderIndex(parsed.data.groupId, parsed.data.orderIndex);
  if (uniqueOrderError) {
    return { success: false, error: uniqueOrderError, fieldErrors: { orderIndex: uniqueOrderError } };
  }
...
export async function updateRoadmapNode(
  ...
  if (!parsed.success) {
    ...
    for (const issue of parsed.error.issues) {
      const key = issue.path[0];
      if (key === "groupId" || key === "nodeId" || key === "lessonId" || key === "orderIndex") {
        fieldErrors[key] = issue.message;
      }
    }
  ...
  if (!existingNode) {
    return { success: false, error: "El nodo no existe o no pertenece al grupo" };
  }
...
export async function deleteRoadmapNode(groupId: string, nodeId: string): Promise<RoadmapNodeActionState> {
  const session = await getSession();
  if (!session.success) {
    return { success: false, error: "No autorizado" };
  }
